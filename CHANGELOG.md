...
- 受限任务队列启用两路并发，可由同一个本地 Qwen 模型同时处理两个任务。
- 终态任务不再被重启前遗留的 Worker 线程覆盖。
- 含未解决条目的任务显示为“部分完成”，并与完整完成分开统计。
- 文本搜索同时支持单文件和目录，文本摘录支持偏移分段读取。
- 超长摘要在本地按 Schema 上限收敛，避免为纯长度问题再次调用模型。
- 向本地 Worker 明确 Hermes 延迟工具的正确调用顺序，减少嵌套误调用。
